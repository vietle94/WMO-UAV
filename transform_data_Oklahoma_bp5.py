...
# %%
dir_in = r"C:\Users\le\OneDrive - Ilmatieteen laitos\WMO-DC\Oklahoma\mavic2pro\bp5/"
...
# Define meta data
operatorID = '049'
airframeID = 'mavic2bp5'
platform_name = 'Mavic 2 bp5'
flight_id = 'Oklahoma_1000m_VP'

# %%
offset = pd.read_excel(r"C:\Users\le\OneDrive - Ilmatieteen laitos\WMO-DC\Oklahoma\mavic2pro\bp5/offset_BP5.xlsx")
offset.columns = ['date', 'flight', 'minute', 'offset']

# %%
for (i, path), (_, offset_) in zip(enumerate(paths[:-2]), offset.iterrows()):
    print(path)
    rename_dict = {
        'latitude' : 'lat',
        'longitude' : 'lon',
        'altitude(meters)' : 'altitude',
        'sht85_temp': 'air_temperature',
        'sht85_rh' : 'relative_humidity',
    ...
    for file in all_files:
        ...
        if 'Flight-Airdata' in os.path.basename(file):
            df = pd.read_csv(file)
            t_delta = datetime.combine(date.min, offset_.offset) - datetime.min
            df['datetime'] = pd.to_datetime(df['datetime(utc)']) + np.timedelta64(1, 'h') - t_delta
        else:
